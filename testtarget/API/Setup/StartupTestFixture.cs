...
namespace APITests.Setup
{
	public class StartupTestFixture
	{
		...
		public DbContextOptions<SprinklerDBContext> DbContextOptions {get;}
		...
		public StartupTestFixture()
		{
			var appSettingBuilder = new ConfigurationBuilder()
				...
				.AddEnvironmentVariables("Sprinkler_")
				.AddEnvironmentVariables($"Sprinkler_Test_");
			...
			DbContextOptions = new DbContextOptionsBuilder<SprinklerDBContext>()
				.UseNpgsql(dbConnectionString).Options;

			PingServer.TestConnection(BaseUrl);
			// % protected region % [Adjust the dbcontext] off begin
			using (var context = new SprinklerDBContext(DbContextOptions, null, null))
			{
