...
namespace Sprinkler.Services.Files.Providers
{
	/// <summary>
	...
	public class FileSystemStorageProvider : IUploadStorageProvider
	{
		...
		public async Task PutAsync(StoragePutOptions options, CancellationToken cancellationToken = default)
		{
			...
			await using var writeStream = File.OpenWrite(GetFileLocation(options.Container, options.FileName));
			await options.Content.CopyToAsync(writeStream, cancellationToken);
