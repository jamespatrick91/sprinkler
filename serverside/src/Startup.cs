...
using Sprinkler.Configuration;
using Sprinkler.Models;
using Sprinkler.Services;
using Sprinkler.Helpers;
using Sprinkler.Utility;
using Sprinkler.Graphql;
using Sprinkler.Graphql.Types;
using Sprinkler.Controllers;
using Sprinkler.Services.Scheduling;
using Sprinkler.Services.Scheduling.Tasks;
using Sprinkler.Services.CertificateProvider;
using Sprinkler.Services.Interfaces;
using Sprinkler.Services.Files;
using Sprinkler.Services.Files.Providers;
using Serilog;
	...
	public class Startup
	{
		...
		public IServiceProvider ConfigureServices(IServiceCollection services)
		{
			...
			Log.Logger = new LoggerConfiguration()
				...
				.Enrich.WithProperty("Application", "Sprinkler")
				.WriteTo.Console()
				.CreateLogger();
			...
			{
				options.InvalidModelStateResponseFactory = ctx => new SprinklerActionResult();
			});
		...
		private void ConfigureDatabaseConnection(IServiceCollection services)
		{
			...
			services.AddDbContext<SprinklerDBContext>(options =>
			{
		...
		private void AddSwaggerService(IServiceCollection services)
		{
			...
			{
				options.SwaggerDoc("json", new OpenApiInfo {Title = "Sprinkler", Version = "v1"});
		...
		private void ConfigureAuthServices(IServiceCollection services)
		{
			...
			services.AddDataProtection()
				.PersistKeysToDbContext<SprinklerDBContext>();
			...
			services.AddIdentity<User, Group>(options =>
				...
				.AddEntityFrameworkStores<SprinklerDBContext>()
			...
			services.AddOpenIddict()
				.AddCore(options =>
				{
					options.UseEntityFrameworkCore()
						.UseDbContext<SprinklerDBContext>()
		...
		private void ConfigureGraphql(IServiceCollection services)
		{
			...
			services.TryAddSingleton<UsersEntityType>();
			services.TryAddSingleton<UsersEntityInputType>();
			services.TryAddSingleton<UserprofilesEntityType>();
			services.TryAddSingleton<UserprofilesEntityInputType>();
			...
			GraphTypeTypeRegistry.Register<UsersEntity, UsersEntityType>();
			GraphTypeTypeRegistry.Register<UserprofilesEntity, UserprofilesEntityType>();
			...
			services.TryAddSingleton<ISchema, SprinklerSchema>();
			services.TryAddSingleton<SprinklerQuery>();
			services.TryAddSingleton<SprinklerMutation>();
			...
			EfGraphQLConventions.RegisterInContainer<SprinklerDBContext>(services);
		...
		private void LoadScheduledTasks(IServiceCollection services)
		{
			...
			// services.TryAddSingleton<IScheduledTask, SomeTask>();
			...
			ILogger<AuditLog> logger)
		{
			...
			{
				options.SwaggerEndpoint("/api/swagger/json/openapi.json", "Sprinkler");
