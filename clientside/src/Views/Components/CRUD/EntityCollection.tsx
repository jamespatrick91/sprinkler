...
import Collection, { ICollectionItemActionProps, expandFn, ICollectionBulkActionProps, ICollectionActionProps } from '../Collection/Collection';
...
class EntityCollection<T extends Model> extends React.Component<IEntityCollectionProps<T>, any> {
	...
	private mapBulkAction = (bulkAction: AdditionalBulkActions<T>): ICollectionBulkActionProps<T> => {
	...
	protected getTableActions = (refetch: () => void) => {
		...
		if (SecurityService.canRead(this.props.modelType) && !this.props.removeViewAction) {
			tableActions.push({
				action: (item) => {
